...
const generateReportContent = (result: AnalysisResult, generatedCode: GeneratedCode, format: 'txt' | 'html'): string => {
  const nl = format === 'txt' ? '\n' : '<br>';
  ...
  if (format === 'html') {
    content += `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Compliance Report for ${result.url}</title>
        <style>
          ...
          .violation { border: 1px solid #ddd; padding: 15px; margin-bottom: 20px; border-radius: 5px; }
  ...
  result.violations.forEach(v => {
    ...
    if (fixes) {
        ...
        Object.entries(fixes).forEach(([framework, data]) => {
...
export const DownloadButtons: React.FC<DownloadButtonsProps> = ({ result, generatedCode }) => {

  const handleDownload = (format: 'txt' | 'html' | 'pdf') => {
    const fileFormat = format === 'pdf' ? 'html' : format; // Use HTML content for PDF
